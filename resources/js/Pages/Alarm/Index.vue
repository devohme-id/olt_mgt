  ...
  <AppLayout>
    <div class="p-6 space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-white">Alarm Management</h1>
          <p class="text-slate-400 text-sm mt-0.5">Monitor and respond to network incidents</p>
        </div>
      </div>

      <!-- Filters -->
      <div class="flex flex-wrap items-center gap-3">
        <select v-model="filters.status" @change="applyFilter" class="px-3 py-2 bg-[#1E293B] border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-amber-500">
          <option value="">All Statuses</option>
          <option value="active">Active</option>
          <option value="acknowledged">Acknowledged</option>
          <option value="cleared">Cleared</option>
        </select>
        <select v-model="filters.severity" @change="applyFilter" class="px-3 py-2 bg-[#1E293B] border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-amber-500">
        ...
        <select v-model="filters.entity_type" @change="applyFilter" class="px-3 py-2 bg-[#1E293B] border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-amber-500">
      ...
      <div class="bg-[#1E293B] rounded-xl border border-slate-700/50 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-700/30">
              <tr v-for="alarm in alarms.data" :key="alarm.id" class="hover:bg-slate-800/30 transition group">
                <td class="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">{{ formatDate(alarm.created_at) }}</td>
                <td class="px-4 py-3">
                  <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold uppercase" :class="severityClass(alarm.severity)">
                    {{ alarm.severity }}
                  </span>
                </td>
                <td class="px-4 py-3 text-white max-w-md truncate" :title="alarm.message">{{ alarm.message }}</td>
                <td class="px-4 py-3">
                  ...
                  <span v-else class="text-slate-300">System</span>
                </td>
        ...
        <!-- Pagination -->
        <div v-if="alarms.last_page > 1" class="flex items-center justify-between px-4 py-3 border-t border-slate-700/50">
          ...
          <div class="flex gap-1">
            <a v-for="link in alarms.links" :key="link.label" :href="link.url || '#'"
              ...
              :class="link.active ? 'bg-amber-500 text-white' : link.url ? 'text-slate-400 hover:bg-slate-700' : 'text-slate-600 cursor-not-allowed'"
              v-html="link.label" />
          </div>
        </div>
      </div>
