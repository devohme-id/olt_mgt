  ...
  <AppLayout>
    <div class="p-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 dark:text-white">OLT Devices</h1>
          <p class="text-slate-500 dark:text-slate-400 text-sm mt-0.5">Manage and monitor your Optical Line Terminals</p>
        </div>
        <a href="/olt/create" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-slate-900 dark:text-white text-sm font-medium rounded-lg transition shadow-lg shadow-blue-500/25">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <div class="flex-1 min-w-[200px] max-w-sm">
          <input
            ...
            class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
        </div>
        ...
        <select v-model="vendorFilter" @change="applyFilter" class="px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-700/30">
              <tr v-for="olt in olts.data" :key="olt.id" class="hover:bg-slate-50 dark:hover:bg-slate-50 dark:bg-slate-800/30 transition-colors group">
                <td class="px-4 py-3">
                  <a :href="'/olt/' + olt.id" class="font-medium text-slate-900 dark:text-white hover:text-blue-400 transition">{{ olt.name }}</a>
                  <p v-if="olt.hostname" class="text-xs text-slate-500 mt-0.5">{{ olt.hostname }}</p>
                </td>
                <td class="px-4 py-3 font-mono text-xs text-slate-700 dark:text-slate-300">{{ olt.ip_address }}</td>
                ...
                <td class="px-4 py-3">
                  <span class="text-slate-700 dark:text-slate-300">{{ olt.vendor?.name || '-' }}</span>
                ...
                <td class="px-4 py-3 text-center">
                  ...
                  <span class="text-slate-700 dark:text-slate-300">{{ olt.onus_count || 0 }}</span>
                </td>
                <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ olt.site?.name || '-' }}</td>
                ...
                <td class="px-4 py-3 text-right">
                  <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition">
                    <button @click.prevent="syncOlt(olt)" title="Sync" class="p-1.5 rounded-md text-slate-500 dark:text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition">
                    ...
                    <a :href="'/olt/' + olt.id + '/edit'" title="Edit" class="p-1.5 rounded-md text-slate-500 dark:text-slate-400 hover:text-amber-400 hover:bg-amber-500/10 transition">
                      <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                    </a>
                    <a :href="'/olt/' + olt.id" title="View" class="p-1.5 rounded-md text-slate-500 dark:text-slate-400 hover:text-green-400 hover:bg-green-500/10 transition">
        ...
        <!-- Pagination -->
        <div v-if="olts.last_page > 1" class="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700/50">
...
const statusClass = (s) => ({
  ...
  'bg-slate-500/10 text-slate-500 dark:text-slate-400': s === 'unknown',
}[true] || 'bg-slate-500/10 text-slate-500 dark:text-slate-400');
