  ...
  <AppLayout>
    <div class="p-6 space-y-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-white">Network Overview</h1>
        <div class="text-sm text-slate-400">
          Last updated: <span class="text-slate-300 font-medium">{{ currentTime }}</span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- OLT Stats -->
        <div class="bg-[#1E293B] rounded-xl border border-slate-700/50 p-5 hover:border-blue-500/50 transition">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-400 text-sm font-medium">OLT Devices</h3>
          ...
          <div class="flex items-end gap-3">
            <span class="text-3xl font-bold text-white">{{ stats.total_olts }}</span>
            <span class="text-sm text-green-400 font-medium mb-1">{{ stats.online_olts }} Online</span>
          </div>
        </div>

        <!-- ONU Stats -->
        <div class="bg-[#1E293B] rounded-xl border border-slate-700/50 p-5 hover:border-cyan-500/50 transition">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-400 text-sm font-medium">ONU Terminals</h3>
          ...
          <div class="flex items-end gap-3">
            <span class="text-3xl font-bold text-white">{{ stats.total_onus }}</span>
            <span class="text-sm text-red-400 font-medium mb-1">{{ stats.offline_onus }} Offline</span>
          </div>
        </div>

        <!-- Alarms -->
        <div class="bg-[#1E293B] rounded-xl border border-slate-700/50 p-5 hover:border-amber-500/50 transition">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-400 text-sm font-medium">Active Alarms</h3>
          ...
          <div class="flex items-end gap-3">
            <span class="text-3xl font-bold text-white">{{ stats.active_alarms }}</span>
            <span class="text-sm text-slate-400 font-medium mb-1">Requires Attention</span>
          </div>
        </div>

        <!-- Health -->
        <div class="bg-[#1E293B] rounded-xl border border-slate-700/50 p-5 hover:border-green-500/50 transition">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-400 text-sm font-medium">Network Health</h3>
          ...
          <div class="flex items-end gap-3">
            <span class="text-3xl font-bold text-white">{{ stats.network_health }}%</span>
            <span class="text-sm text-slate-400 font-medium mb-1">Overall Stability</span>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- OLT List -->
        <div class="lg:col-span-2 bg-[#1E293B] rounded-xl border border-slate-700/50">
          <div class="p-4 border-b border-slate-700/50 flex items-center justify-between">
            <h2 class="text-white font-semibold">Managed OLTs</h2>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
              ...
              <tbody class="divide-y divide-slate-700/30">
                <tr v-for="olt in olts" :key="olt.id" class="hover:bg-slate-800/50 transition">
                  <td class="px-4 py-3">
                    <a :href="'/olt/' + olt.id" class="font-medium text-white hover:text-blue-400">{{ olt.name }}</a>
                  </td>
                  <td class="px-4 py-3 font-mono text-slate-300 text-xs">{{ olt.ip_address }}</td>
                  <td class="px-4 py-3 text-slate-400">{{ olt.vendor?.name }}</td>
                  <td class="px-4 py-3">
                    <span class="inline-flex items-center gap-1.5">
                      <span class="w-2 h-2 rounded-full" :class="olt.status === 'online' ? 'bg-green-400' : 'bg-red-400'"></span>
                      <span class="capitalize text-slate-300">{{ olt.status }}</span>
                    </span>
                  </td>
                </tr>
        ...
        <div class="bg-[#1E293B] rounded-xl border border-slate-700/50">
          <div class="p-4 border-b border-slate-700/50 flex items-center justify-between">
            <h2 class="text-white font-semibold">Recent Alarms</h2>
          ...
          <div class="p-4 flex flex-col items-center justify-center h-[300px] text-center">
            ...
            <p class="text-slate-400 font-medium">No Active Alarms</p>
